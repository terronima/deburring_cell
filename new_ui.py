...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.LR_add_parts = QtWidgets.QPushButton(self.groupBox_8)
        self.LR_add_parts.setObjectName("LR_add_parts")
        self.gridLayout_3.addWidget(self.LR_add_parts, 3, 3, 1, 1)
        self.LR_remove_parts = QtWidgets.QPushButton(self.groupBox_8)
        self.LR_remove_parts.setObjectName("LR_remove_parts")
        self.gridLayout_3.addWidget(self.LR_remove_parts, 3, 5, 1, 1)
        ...
        self.SR_add_parts = QtWidgets.QPushButton(self.groupBox_7)
        self.SR_add_parts.setObjectName("SR_add_parts")
        self.gridLayout_4.addWidget(self.SR_add_parts, 3, 3, 1, 1)
        self.SR_remove_parts = QtWidgets.QPushButton(self.groupBox_7)
        self.SR_remove_parts.setObjectName("SR_remove_parts")
        self.gridLayout_4.addWidget(self.SR_remove_parts, 3, 5, 1, 1)
        ...
        self.SR_add_parts.clicked.connect(lambda: self.SR_update_num(num=1))
        self.SR_remove_parts.clicked.connect(lambda: self.SR_update_num(num=-1))
        self.LR_add_parts.clicked.connect(lambda: self.LR_update_num(num=1))
        self.LR_remove_parts.clicked.connect(lambda: self.LR_update_num(num=-1))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.LR_add_parts.setText(_translate("MainWindow", "+"))
        self.LR_remove_parts.setText(_translate("MainWindow", "-"))
        ...
        self.SR_add_parts.setText(_translate("MainWindow", "+"))
        self.SR_remove_parts.setText(_translate("MainWindow", "-"))
    ...
    def SR_update_num(self, num):
        curr_num = int(self.SR_num_of_repeats.text()) + num
        if curr_num > 0:
            self.SR_num_of_repeats.setText(str(curr_num))

    def LR_update_num(self, num):
        curr_num = int(self.LR_num_of_repeats.text()) + num
        if curr_num > 0:
            self.LR_num_of_repeats.setText(str(curr_num))
    ...
    def num_before_reaccess(self, robot):
        # how many parts should be processed before touch seq will be executed again
        if robot == "sr":
            num = self.SR_num_of_repeats.text()
            self.send(f"HMI,r2,steps_{num}")
        else:
            num = self.LR_num_of_repeats.text()
            self.send(f"HMI,r1,steps_{num}")

    def status_updates(self, incoming):
        recv = incoming
        print(f"from status update recv is {recv}")
        if recv in ["number_of_cycles_lr", "number_of_cycles_sr"]:
            print(recv[17::])
            self.num_before_reaccess(recv[17::])
        if recv == "name":
            data = "HMI"
            self.send(f"{data}")
            print(f"Sent: {data}")
...
# starting the app
sys.exit(app.exec_())
# /home/triton/Documents/22011 - Vision Code/logo.png